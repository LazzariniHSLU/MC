...
void ifrRxFrontInit(void)
{
  // @ToDo 1.) complete TPM1CH0 init.
	TPM1SC = 0; // Reset
	TPM1SC_CLKSA = 1; // Clock Source = Bus rate clock
	TPM1C0SC = 0; // Reset (set to Input Capture)
	TPM1C0SC_ELS0B = 1; // Capture on any edges
	TPM1C0SC_ELS0A = 1; // Capture on any edges
	TPM1C0SC_CH0F = 0; // Clear interrupt flag
	TPM1C0SC_CH0IE = 1; // Interrupt enabled
}
